...
export const AdOverlay = ({ open, onClose, onAdComplete }: AdOverlayProps) => {
  ...
  const [consentRequired, setConsentRequired] = useState(() => !localStorage.getItem('ad-consent-granted'));

  const canShowAd = useMemo(() => !consentRequired, [consentRequired]);

  useEffect(() => {
    let timer: NodeJS.Timeout;
    if (open && canShowAd && countdown > 0) {
      timer = setInterval(() => {
        setCountdown((prev) => prev - 1);
      }, 1000);
    }
    return () => clearInterval(timer);
  }, [open, countdown, canShowAd]);
  ...
  return (
    <>
      <Dialog open={open && !consentRequired} onOpenChange={(val) => !val && countdown === 0 && onClose()}>
      ...
      {open && consentRequired && (
        <Dialog open={true} onOpenChange={() => onClose()}>
          <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none bg-transparent">
            <CookieConsent onAccept={() => setConsentRequired(false)} />
          </DialogContent>
        </Dialog>
      )}
    </>
